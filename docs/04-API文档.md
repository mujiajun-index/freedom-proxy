...
带 `Upgrade: websocket` 的升级请求同样按映射前缀匹配：

- 命中映射 → 与目标建立连接（目标 `https://` 走 `wss://`/TLS，`http://` 走 `ws://`/明文）。
- **握手透传**：客户端的 `Sec-WebSocket-Key` 等头透传给目标，目标的 `101 Switching Protocols`（含 `Sec-WebSocket-Accept`、扩展头）原样回给客户端。
- 握手成功后，客户端与目标之间为**透明双向裸流**，WS 帧不解析、原样转发（子协议、permessage-deflate 等由端到端协商）。
- 受 IP 白名单约束；升级成功会在访问日志记一条 `status=101`、`streamType=stream`。
- 未命中映射 → `404`；白名单拒绝 → `403`；上游握手超时 → `504`；上游连接失败 → `502`。

> 客户端需用与映射前缀一致的路径连接，例如映射 `/ws` → 目标，则客户端连 `ws(s)://你的域名/ws`。
